...
from settings import RIDE_SCHEMA, CONSUME_TOPIC_RIDES_FHV_CSV, CONSUME_TOPIC_RIDES_GREEN_CSV, TOPIC_WINDOWED_VENDOR_ID_COUNT, TOPIC_RIDES_ALL
...
if __name__ == "__main__":
    ...
    # read_streaming data
    df_consume_fhv_stream = read_from_kafka(consume_topic=CONSUME_TOPIC_RIDES_FHV_CSV)
    print(df_consume_fhv_stream.printSchema())
    df_consume_green_stream = read_from_kafka(consume_topic=CONSUME_TOPIC_RIDES_GREEN_CSV)
    print(df_consume_green_stream)
    ...
    df_trip_count_messages = prepare_df_to_kafka_sink(df=df_trip_count_by_pulocationid,
                                                      value_columns=['count'], key_column='PULocationID')
    df_all_rides_messages = prepare_df_to_kafka_sink(df=df_rides, 
                                                     value_columns=['PULocationID'], key_column='PULocationID')
    kafka_sink_query = sink_kafka(df=df_trip_count_messages, topic=TOPIC_WINDOWED_VENDOR_ID_COUNT)
    #kafka_sink_all_rides = sink_kafka(df=df_all_rides_messages, topic=TOPIC_RIDES_ALL)
